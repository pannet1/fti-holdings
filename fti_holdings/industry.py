...
industry_stocks = stock_data[stock_data["industry"] == industry_of_interest.lower()]
...
merged_data["12dayma"] = (
    merged_data.groupby("symbol")["changepercentage"]
    .rolling(window=12)
    .mean()
    .reset_index(level=0, drop=True)
)
for symbol, data in merged_data.groupby("symbol"):
    plt.plot(data["date"], data["12dayma"], linestyle="--", label=f"{symbol} 12-Day MA")
...
for symbol, data in merged_data.groupby("symbol"):
    ...
    ax.plot(data["date"], data["open"], label="Open", linestyle="-", marker="o")
    ax.plot(data["date"], data["high"], label="High", linestyle="-", marker="o")
    ax.plot(data["date"], data["low"], label="Low", linestyle="-", marker="o")
    ax.plot(data["date"], data["close"], label="Close", linestyle="-", marker="o")
