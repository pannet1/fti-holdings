...
`broker-ai` replaces `omspy-brokers` and `stock-brokers` as the bridge between the ratchet strategy and Finvasia (Shoonya) broker API.
...
| Aspect | Zerodha (old) | Finvasia (new) |
|--------|--------------|----------------|
| Auth | API key + secret + enctoken | userid + password + TOTP |
| Token persistence | enctoken file (daily expiry) | Session token (daily) |
| Instrument lookup | `kite.instruments()` | `NorenApi.searchscrip()` |
| Order types | `LIMIT`, `MARKET`, `SL`, `SL-M` | Same |
| Product codes | `CNC`, `MIS`, `NRML` | `C`, `M`, `NRML` |
| Exchange codes | `NSE`, `BSE`, `NFO` | Same |
| WebSocket | Kite Connect Ticker | Shoonya WebSocket |
| Historical data | `kite.historical_data()` | `NorenApi.get_time_price_series()` |
...
### bypass.yaml (Finvasia credentials)
...
1. **Unit tests**: Mock broker-ai responses, test signal logic
2. **Integration tests**: Real broker-ai calls with paper trading account
3. **Functional tests**: Full end-to-end with Playwright (server only)
