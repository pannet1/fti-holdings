...
from .Handler import HoldingsTrackerHandler
from .Schema import HoldingsRow


class TestHoldingsTrackerHandler:

    def test_read_all_returns_empty_when_file_missing(self, tmp_path):
        handler = HoldingsTrackerHandler(data_dir=str(tmp_path))
    ...
    def test_read_all_returns_all_rows(self, tmp_path):
        ...
        csv_path.write_text(
            ...
            "2024-01-15 11:00:00,BSE,MOTHERSON,BUY,180.75,33,ratchet\n"
        )
        handler = HoldingsTrackerHandler(data_dir=str(tmp_path))
        result = handler.read_all()
        assert len(result) == 2
        assert result[0].tradingsymbol == "ITBEES"
        assert result[1].tradingsymbol == "MOTHERSON"
    ...
    def test_read_by_symbol_filters_correctly(self, tmp_path):
        ...
        motherson = handler.read_by_symbol("MOTHERSON")
        assert len(itbees) == 2
        assert len(motherson) == 1
        assert all(r.tradingsymbol == "ITBEES" for r in itbees)
        assert all(r.tradingsymbol == "MOTHERSON" for r in motherson)
    ...
    def test_remove_holding_does_nothing_for_nonexistent_symbol(self, tmp_path):
        ...
        handler.remove_holding("MOTHERSON", 33)
        rows = handler.read_all()
        assert len(rows) == 1
        assert rows[0].tradingsymbol == "ITBEES"
